    ...
    while (location != NULL) {
        ...
        else
        {
            ...
            if (item < location->info) {
                std::cout << "ERROR: Not In the List1\n";
                break;
            }
        }
    }
    if (location == NULL) {
        std::cout << "ERROR: NOT In The List2\n";
    }
}
template <class ItemType>
...
void SortedType<ItemType> ::MergeLists(SortedType<ItemType>& other, SortedType<ItemType>& result) {
    NodeType<ItemType>* one = listData;
    NodeType<ItemType>* two = other.listData;

    while (one != NULL || two != NULL) {
        if (one == NULL && two != NULL) {
            result.InsertItem(two->info);
            two = two->next;
        }
        else if (one != NULL && two == NULL) {
            result.InsertItem(one->info);
            one = one->next;
        }
        else {
